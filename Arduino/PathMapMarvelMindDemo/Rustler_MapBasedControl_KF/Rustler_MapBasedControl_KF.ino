...
void setup() {
  ...
  //kf takes U, X, Y, Psi, XYvar, psivar
  kf.begin(0,0,0,0,.02,.01);
...
//      steerServo.write(steerVal);
  ...
  server.on("/data", HTTP_GET, [](AsyncWebServerRequest * req) {
    ...
    String res = "{";
    res += "\"x\":" + String(xfilt, 3) + ",";
    res += "\"y\":" + String(yfilt, 3) + ",";
    res += "\"yaw\":" + String(yawfilt, 3) + ",";
...
void loop() {
  ...
  beacon.update();
  bool xy_avail = beacon.positionAvailable();
  ...
  kf.update(micros(),yawrate,xy_avail,x,-y,yaw);
  xfilt = kf.getX();
  yfilt = kf.getY();
  yawfilt = kf.getPsi();
  U = kf.getU();
...
//    float station = path.estimateStation(x, -y, yaw);
    station = path.estimateStation(xfilt,yfilt,yawfilt);
    ePrev   = path.previewLateralError(xfilt, yfilt, yawfilt, previewDist);
    gasServo.write(gasVal);
    float autoSteer = 90-previewGain*ePrev;
    autoSteer = constrain(autoSteer,30,150);
...
//  gasServo.write(gasVal);
  ...
  Serial.print(xfilt); Serial.print(", ");
  Serial.print(yfilt); Serial.print(", ");
  Serial.print(yawfilt); Serial.print(", ");
